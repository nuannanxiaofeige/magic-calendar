/**
 * 数据库迁移并同步天行 API 数据
 * 用法：node scripts/migrate_and_sync.js [year]
 * 例如：node scripts/migrate_and_sync.js 2026
 */
...
async function main() {
  ...
  if (!year || isNaN(year)) {
    ...
    console.error('用法：node scripts/migrate_and_sync.js [year]')
    console.error('例如：node scripts/migrate_and_sync.js 2026')
    process.exit(1)
  }

  console.log('========================================')
  console.log(`开始执行数据库迁移并同步 ${year}年 数据`)
  console.log('========================================')
  ...
  try {
    ...
    // 1. 执行表结构迁移
    console.log('\n[1/3] 执行表结构迁移...')
    const sqlFilePath = path.join(__dirname, '../sql/holidays redesign.sql')
    console.log(`  执行 SQL 文件：${sqlFilePath}`)
    ...
    console.log(`  已创建表：${tables.map(t => t.table_name).join(', ')}`)
    ...
    const [holidayCount] = await connection.query(
      'SELECT COUNT(*) as count FROM holidays WHERE year = ?',
      [year]
    )

    const [termCount] = await connection.query(
      'SELECT COUNT(*) as count FROM almanac_term_dates WHERE year = ?',
      [year]
    )

    console.log(`  holidays 表中 ${year}年 数据：${holidayCount[0].count} 条`)
    console.log(`  almanac_term_dates 表中 ${year}年 数据：${termCount[0].count} 条`)

    console.log('\n========================================')
    console.log(`迁移和同步完成！${year}年 数据已写入数据库`)
    console.log('========================================')
